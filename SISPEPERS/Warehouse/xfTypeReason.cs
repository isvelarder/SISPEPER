...
{
    public partial class xfTypeReason : XtraForm
    {
        public xfTypeReason()
        {
            InitializeComponent();
        ...
        private void Set_Operation(int index)
        {
            try
            {
                if (index == 0)
                {
                    ...
                    gdvGeneric.RefreshData();
                    gdvGeneric.MoveLast();
                    gdvGeneric.FocusedColumn = gcALF_TIPO_MOTI;
                    gdvGeneric.ShowEditor();
                }
                else if (index == 1)
                {
                    var row = (BETypeReason)gdvGeneric.GetRow(gdvGeneric.FocusedRowHandle);
                    if (row.IND_MNTN != 0 &&
                        row.IND_MNTN != 3)
                    {
        ...
        {
            var obej = (BETypeReason)gdvGeneric.GetRow(e.RowHandle);
            if (obej.IND_MNTN == 3)
            {
