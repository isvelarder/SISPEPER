...
namespace SISPEPERS
{
    public partial class xfTransferReceivingGoodsSearch : XtraForm
    {
        public xfTransferReceivingGoodsSearch()
        {
            InitializeComponent();
        }

        public BETransferGoods rowsel { get; set; }
        private void Search()
        {
            try
            {
                var opar = new BETransferGoods() 
                { 
                    COD_ALMA = (int?)lkeCOD_ALMA.EditValue, 
                    COD_MOTI = (int?)lkeCOD_MOTI.EditValue, 
                    FEC_TRAN = (DateTime?)dteFEC_SALI.EditValue, 
                ...
                var olst = obr.Get_PSGN_SPLS_SVTC_ALMA_TRAN_RECE(opar);
        ...
        private void Get_RowSelecion()
        {
            try
            {
                ...
                rowsel = (BETransferGoods)gdvSearch.GetRow(gdvSearch.FocusedRowHandle);
