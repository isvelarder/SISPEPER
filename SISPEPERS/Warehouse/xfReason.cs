...
{
    public partial class xfReason : XtraForm
    {
        public xfReason()
        {
            InitializeComponent();
        ...
        private void Set_Operation(int index)
        {
            try
            {
                if (index == 0)
                {
                    ...
                    var olst = (List<BEReason>)gdvGeneric.DataSource;
                    var i = 1;
                    olst.ForEach(obej => 
                    {
                    ...
                    gdvGeneric.RefreshData();
                    gdvGeneric.MoveLast();
                    gdvGeneric.FocusedColumn = gcALF_MOTI;
                    gdvGeneric.ShowEditor();
                }
                else if (index == 1)
                {
                    var row = (BEReason)gdvGeneric.GetRow(gdvGeneric.FocusedRowHandle);
                    if (row.IND_MNTN != 0 &&
                        row.IND_MNTN != 3)
                    {
        ...
        private void Set_Save()
        {
            ...
            try
            {
                ...
                var obr = new BRReason();
                var obej = new BEReason();
                olst.ForEach(item => item.COD_COMP = xfMain.SgIns.SESSION_COMP);
                obr.Set_PSGN_SPMT_SVMC_MOTI(obej, olst);
        ...
        {
            var obej = (BEReason)gdvGeneric.GetRow(e.RowHandle);
            if (obej.IND_MNTN == 3)
            {
