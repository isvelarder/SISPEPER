...
{
    public partial class xfCreditNote : XtraForm
    {
        ...
        /// </summary>
        private static xfCreditNote mSgIns;
        public static xfCreditNote SgIns
        {
            get
            {
                if (mSgIns == null || mSgIns.IsDisposed)
                    mSgIns = new xfCreditNote();
                return (mSgIns);
            }
            set
            {
        ...
        private void xfCreditNote_Load(object sender, EventArgs e)
        {
            Get_Load();            
        }
        ...
        private void xfCreditNote_Activated(object sender, EventArgs e)
        {
        ...
        private void xfCreditNote_Deactivate(object sender, EventArgs e)
        {
            foreach (BarButtonItemLink itemLink in ((xfMain)MdiParent).barTool.ItemLinks)
                itemLink.Item.Visibility = BarItemVisibility.Never;
        }

        private void xfCreditNote_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        public void Set_Save()
        {
            ...
            try
            {
                ...
                var obr = new BRPurchase();
                obr.Set_PSCP_SPMT_SCPC_ORPC(odoc, lines);
                ...
                gdvLines.RefreshData();
                StateControls(true);
                xfInvoice.SgIns.lkeIND_ESTA.EditValue = "CE";
        ...
        private void Set_CreditNote(BEDocument row)
        {
            ClearControls();
            ...
            var obr = new BRPurchase();
            var olst = obr.Get_PSCP_SPLS_SCPD_ORPC(obj);
        ...
        {
            var ofx = new xfCreditNoteSearch() { SESSION_COMP = SESSION_COMP };
            if (ofx.ShowDialog() == DialogResult.OK)
                Set_CreditNote(ofx.rowsel);
        }
