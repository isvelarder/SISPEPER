...
{
    public partial class xfDelivery : XtraForm
    {
        ...
        /// </summary>
        private static xfDelivery mSgIns;
        public static xfDelivery SgIns
        {
            get
            {
                if (mSgIns == null || mSgIns.IsDisposed)
                    mSgIns = new xfDelivery();
                return (mSgIns);
            }
            set
            {
        ...
        {
            olgn = new List<BEDocumentExpenses>();
        ...
        private void HandleSendCopyToDeliveryClick(object sender, ItemClickEventArgs e)
        {
            try
            {
                ...
                xfInvoice.SgIns.docb = odoc;
                xfInvoice.SgIns.MdiParent = MdiParent;
                xfInvoice.SgIns.Activate();
                xfInvoice.SgIns.FORM_SUBO = "bbiPInvoice";
                xfInvoice.SgIns.Show();
                xfInvoice.SgIns.BringToFront();
            }
            catch (Exception ex)
            {
        ...
        private void Set_DocumentBase()
        {
            ...
            var obj = new BEDocument() { COD_ORCO = docb.COD_ORCO };
            var obr = new BRPurchase();
            var olst = obr.Get_PSCP_SPLS_SCPD_ORCO(obj);
            olst.ForEach(item => 
            {
                item.TIP_DOCU_BASE = 1;
                item.COD_DOCU_BASE = item.COD_ORCO;
        ...
        private void xfDelivery_Load(object sender, EventArgs e)
        {
            Get_Load();            
        }
        ...
        private void xfDelivery_Activated(object sender, EventArgs e)
        {
        ...
        private void xfDelivery_Deactivate(object sender, EventArgs e)
        {
            foreach (BarButtonItemLink itemLink in ((xfMain)MdiParent).barTool.ItemLinks)
                itemLink.Item.Visibility = BarItemVisibility.Never;
        }

        private void xfDelivery_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        public void Set_Save()
        {
            ...
            try
            {
                ...
                var obr = new BRPurchase();
                obr.Set_PSCP_SPMT_SCPC_GUIA(odoc, lines, olgn);
                ...
                gdvLines.RefreshData();
                StateControls(true);
                xfPurchaseOrder.SgIns.lkeIND_ESTA.EditValue = "CE";
        ...
        private void Set_Delivery(BEDocument row)
        {
            ClearControls();
            ...
            StateControls(true);
            //sbtExpen.Enabled = false;
            txtCOD_DOCU.Focus();
        }
        public void Set_Search()
        {
            var ofx = new xfDeliverySearch() { SESSION_COMP = SESSION_COMP };
            if (ofx.ShowDialog() == DialogResult.OK)
                Set_Delivery(ofx.rowsel);
        }
