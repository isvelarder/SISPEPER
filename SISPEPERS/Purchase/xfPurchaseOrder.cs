...
{
    public partial class xfPurchaseOrder : XtraForm
    {
        public xfPurchaseOrder()
        {
            InitializeComponent();
        }
        ...
        /// </summary>
        private static xfPurchaseOrder mSgIns;
        public static xfPurchaseOrder SgIns
        {
            get
            {
                if (mSgIns == null || mSgIns.IsDisposed)
                    mSgIns = new xfPurchaseOrder();
                return (mSgIns);
            }
            set
            {
        ...
        private void HandleSendCopyToDeliveryClick(object sender, ItemClickEventArgs e)
        {
            try
            {
                ...
                {
                    COD_ORCO = Convert.ToInt32(txtCOD_ORCO.Text),
                ...
                xfDelivery.SgIns.docb = odoc;
                xfDelivery.SgIns.MdiParent = MdiParent;
                xfDelivery.SgIns.Activate();
                xfDelivery.SgIns.FORM_SUBO = "bbiInputItems";
                xfDelivery.SgIns.Show();
                xfDelivery.SgIns.BringToFront();
            }
            catch (Exception ex)
            {
        ...
        private void xfPurchaseOrder_Load(object sender, EventArgs e)
        {
            Get_Load();
            StateControls(true);
        }
        private void lkeCOD_MONE_EditValueChanged(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                txtNUM_TIPO_CAMB.Properties.ReadOnly = (string.IsNullOrWhiteSpace(txtCOD_ORCO.Text)) ? false: true;
        ...
        private void xfPurchaseOrder_Activated(object sender, EventArgs e)
        {
        ...
        private void xfPurchaseOrder_Deactivate(object sender, EventArgs e)
        {
            foreach (BarButtonItemLink itemLink in ((xfMain)MdiParent).barTool.ItemLinks)
                itemLink.Item.Visibility = BarItemVisibility.Never;
        }

        private void xfPurchaseOrder_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        public void Set_Save()
        {
            ...
            try
            {
                ...
                var odoc = new BEDocument()
                {
                    COD_ORCO = (!string.IsNullOrWhiteSpace(txtCOD_ORCO.Text)) ? Convert.ToInt32(txtCOD_ORCO.Text) : (int?)null,
                    ALF_DOCU_REFE = txtALF_DOCU_REFE.Text.Trim(),
                    IND_ESTA = lkeIND_ESTA.EditValue.ToString(),
                    COD_SUCU = SESSION_COMP,
                    IND_TIPO_COMP = rdgIND_TIPO_COMP.EditValue.ToString(),
                    ...
                    IND_MNTN = (!string.IsNullOrWhiteSpace(txtCOD_ORCO.Text)) ? 2 : 1
                };
        ...
        public void Edit()
        {
            try
            {
                if (string.IsNullOrEmpty(txtCOD_ORCO.Text))
                    throw new ArgumentException(_Message.MsgSelectOC);
                if (lkeIND_ESTA.EditValue.ToString() != "AB")
                    throw new ArgumentException(string.Format("{0}{1}.", _Message.MsgNotModyDocuSta, lkeIND_ESTA.Text.ToLower()));
                StateControls(false);
                bteCOD_SOCI_NEGO.Focus();
            }
            catch (Exception ex)
        ...
        private void Set_PurchaseOrder(BEDocument row)
        {
            ClearControls();
            txtCOD_ORCO.Text = row.COD_ORCO.ToString();
            ...
            var obj = new BEDocument() { COD_ORCO = row.COD_ORCO };
            var obr = new BRPurchase();
            var olst = obr.Get_PSCP_SPLS_SCPD_ORCO(obj);
        ...
        {
            var ofx = new xfPurchaseOrderSearch() { SESSION_COMP = SESSION_COMP };
            if (ofx.ShowDialog() == DialogResult.OK)
                Set_PurchaseOrder(ofx.rowsel);
        }
